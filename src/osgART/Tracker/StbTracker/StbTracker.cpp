...
inline osgART::Marker* TrackerStb::addMarker(const std::string& config) {
	...
	if (markerType == "ID") {
		...
		if (tokens.size() == 3) {
			...
			m_markerlist.push_back(markerStb);
...
inline void TrackerStb::update(osg::NodeVisitor* nv) {
	...
	// Update the image
	if (!m_imagesource.valid()) return;
	image->setPixels(m_imagesource->data(), m_imagesource->s(), m_imagesource->t(), StbCore::PIXEL_FORMAT_BGRA);
