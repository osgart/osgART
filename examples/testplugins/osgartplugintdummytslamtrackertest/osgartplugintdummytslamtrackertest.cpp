...
int main(int argc, char* argv[])  {
	...
	//if you SLAM tracker use rgbd, you can load a multi stream video plugin such as openni, pcl, etc
	//osgART::PluginManager::instance()->load("osgart_video_dummyrgbdvideo");

	//tracker plugin
	osgART::PluginManager::instance()->load("osgart_tracker_dummyslamtracker");
	...
	osg::ref_ptr<osgART::VisualTracker> tracker 
		= dynamic_cast<osgART::VisualTracker*>(osgART::PluginManager::instance()->get("osgart_tracker_dummyslamtracker"));
	...
	// setup one target: here it's a slam target, which expose getGeometry() function
	//we need to cast it since we will call the function getGeometry from SLAMTarget
	osg::ref_ptr<osgART::SLAMTarget> target =dynamic_cast<osgART::SLAMTarget*>(tracker->addTarget());

	//osg::ref_ptr<osgART::SLAMTarget> target =dynamic_cast<osgART::SLAMTarget*>(tracker->addTarget("test.pattern;35.2;22.0;0.3"));
	...
	//create geode to handle geometry produced by SLAM tracker
	...
	arTransform->addChild(mygeode.get());
